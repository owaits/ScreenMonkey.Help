...
The way Screen Monkey formats dates and time is based on a common set of letter abreviations which are listed in full below together with examples. Take care to understand how these work and when the special escape character is required.
...
|dd|Day of the month in decimal format with leading zeros|01 to 31|
|ddd|Day of the week, abreviated|Tue|
|dddd|Day of the week|Tuesday|
|M|Month of the year in decimal format|1 to 12|
|MM|Month of the year in decimal format with leading zeros|01 to 12|
...
To identify character as a regular letter instead of a placeholder you place a backslash \ before it. To make the example above work the following need to be escaped - H, t, s, h, m. So it must be re-written as `\Here i\s \t\he \ti\me: HH:mm` and `Nex\t \Mee\tin\g a\t HH:mm`
