...
This topic explains the concepts relating to the **Effect Generator** clip type, and how to create your own effects scripts. For reference information on the effect generator scripting language take a look at the [Scripting Language Reference](ScriptingLanguageReference.md).
...
Click Update Preview and observe the preview window to see the results of this change. We now see two lines on screen at the same time because one new LINE is being generated each cycle, and each LINE lives for two cycles.
...
**Note:** for consistency with other scripting languages, the American spelling of "color" is used for color scheme definitions.
